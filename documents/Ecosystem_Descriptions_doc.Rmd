...
```{r calculate abiotic means, message=FALSE, warning=FALSE, include=FALSE}
 unit_cont <- ecodat %>% filter(name %in% abiotic) %>% group_by(name) %>% 
 ...
 envgr_cont <- ecodat %>% filter(name %in% abiotic) %>% group_by(name) %>% 
  ...
  unit_envgr <- ecodat %>% filter(name %in% abiotic & grepl(pattern = word(unitname, 1,1, "B"), unit_h)) %>% group_by(name) %>% 
    summarise(mean_diff = mean(value[unit_h == unitname], na.rm = T) - mean(value[unit_h != unitname], na.rm = T),
              medi_diff = median(value[unit_h == unitname], na.rm = T) - median(value[unit_h != unitname], na.rm = T)) %>% arrange(desc(mean_diff))
...
ecodat$f <- "continent"
ecodat$f[grepl(pattern = word(unitname, 1,1, "B"), ecodat$unit_h)] <- "env. unit"
ecodat$f[ecodat$unit_h == unitname] <- "assemblage"

ggplot(ecodat %>% filter(name %in% abiotic), aes(y = value, x = f, fill = f)) + 

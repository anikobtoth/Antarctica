 ...
 # Full map
  ggplot() +  
    geom_sf(data=antarctica, fill="gray50", color= NA, size=0.25) + coord_sf(datum = st_crs(3031)) +
     geom_tile(data = typ_df, aes(x = x, y = y), fill = "gray35", col = "gray35", lwd = 0.4) +
...
```{r inset maps, echo=FALSE, message=FALSE, warning=FALSE}
  ...
  subplots <- map2(subplots, units, function(x, y){
    p <- x + geom_tile(data = y, aes(x=x, y=y, col= ecosystem, fill = ecosystem), lwd = .6) +
      scale_fill_manual(values = c("#E0002D")) + scale_color_manual(values = c("#E0002D"))
    return(p)
  })
...
ggplot(ecodat %>% filter(name %in% abiotic), aes(y = value, x = f, fill = f)) + 
  geom_boxplot(outlier.size = 0.2) + facet_wrap(.~name, scales = "free_y") + labs(x = "Abiotic group", fill = "Abiotic group")
